...
public abstract class Scsat1OngoingTransfer implements CfdpFileTransfer {
    ...
    private Tuple toTuple(CfdpTransactionId id, byte[] packet) {
    ...
    protected void sendPacket(CfdpTransactionId id, byte[] packet) {
        if (log.isDebugEnabled()) {
            log.debug("TXID{} sending PDU: {}", cfdpTransactionId, packet);
            log.trace("{}", StringConverter.arrayToHexString(packet, true));
        }
        cfdpOut.emitTuple(this.toTuple(id, packet));
    }
